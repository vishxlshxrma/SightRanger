        ...
        for index in range(0, len(frames), frames_batch_size):
            ...
            if intrusion_detected:
                email_service.send_notification(
                    recipient=recipient,
                    title="Intrusion Detected - SightRanger",
                    subject=f"Intrusion Detected at {datetime.now().strftime('%d/%m/%Y %H:%M:%S')}",
                    message="We have detected an intrusion on your SightRanger surveillance system. Please find the video file attached for your reference.",
    ...
    try:
        ...
        for index in range(0, len(frames), frames_batch_size):
            ...
            if intrusion_detected and weapon_detected:
                email_service.send_notification(
                    recipient=recipient,
                    title="Threat Detected - SightRanger",
                    subject=f"Threat Detected at {datetime.now().strftime('%d/%m/%Y %H:%M:%S')}",
                    message="We have detected a threat on your SightRanger surveillance system. Please find the video file attached for your reference.",
                    download_link=f"{settings.backend_api_url}/files/download/{file_id}",
                )
                break
